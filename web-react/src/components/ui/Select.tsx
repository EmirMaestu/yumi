...
export default function Select({ value, onValueChange, options, placeholder, ariaLabel, style }: { value?: string; onValueChange: (v: string) => void; options: Opt[]; placeholder?: string; ariaLabel?: string; style?: React.CSSProperties }) {
  return (
    <RS.Root value={value} onValueChange={onValueChange}>
      ...
      <RS.Portal>
        <RS.Content position="popper" sideOffset={6} className="nf-pop" style={{ background: 'var(--color-linen)', border: '1px solid var(--color-mist)', borderRadius: 12, padding: 6, zIndex: 70, boxShadow: '0 8px 28px rgba(18,22,19,0.14)', minWidth: 'max(var(--radix-select-trigger-width), 180px)' }}>
          <RS.Viewport>
            {options.map((o) => (
